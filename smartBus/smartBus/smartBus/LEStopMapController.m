//
//  LEStopMapController.m
//  smartBus
//
//  Created by Eda on 2017/5/9.
...
@interface LEStopMapController ()<MAMapViewDelegate, PlaceAroundTableViewDeleagate>
...
@implementation LEStopMapController
...
- (void)mapView:(MAMapView *)mapView didUpdateUserLocation:(MAUserLocation *)userLocation updatingLocation:(BOOL)updatingLocation
{
    ...
    if (!self.isLocated)
    {
        ...
        double latitude = [NSString stringWithFormat:@"%@",self.stopData[@"stop_latitude"]].doubleValue;
        double longitude = [NSString stringWithFormat:@"%@",self.stopData[@"stop_longitude"]].doubleValue;
        CLLocationCoordinate2D coords = CLLocationCoordinate2DMake(latitude,longitude);

        
        [self.mapView setCenterCoordinate:CLLocationCoordinate2DMake(coords.latitude, coords.longitude)];
        
        [self actionSearchAroundAt: coords];
//        
...
- (void)initMapView
{
    self.mapView = [[MAMapView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.view.bounds), self.view.bounds.size.height/3*2)];
...
- (void)initTableview
{
    self.tableview = [[PlaceAroundTableView alloc] initWithFrame:CGRectMake(0, self.view.bounds.size.height/3*1.76, CGRectGetWidth(self.view.bounds), self.view.bounds.size.height/3*1.24)];
...
-(void) loadStopDetail{
    ...
    // 3.文件路径
    NSString *fileName = [NSString stringWithFormat:@"stop_detail_%@.plist", self.stopId];
    NSString *filepath = [docPath stringByAppendingPathComponent:fileName];
    
   
    NSDictionary *stopDetail= [[NSDictionary alloc] initWithContentsOfFile:filepath];
    
    self.stopData = stopDetail;
...
- (void)viewDidLoad
{
    ...
    [self loadStopDetail];
    [self initTableview];
    self.title =  [NSString stringWithFormat:@"%@", self.stopData[@"stop_name"]];
